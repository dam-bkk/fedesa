...
export default async function FederationPage() {
  ...
  return (
    <main>
      ...
      <Federation  t={t} />
      <section id="bureau" className="tight"><div className="wrap"><div className="sec-head"><div><div className="eyebrow">{t("Élus pour l'olympiade")}</div><h2 style={{ marginTop: 14 }}>{t("Bureau")}<br />{t("fédéral")}</h2></div></div>
      ...
      <Timeline />
      <section id="ligues" className="tight"><div className="wrap"><div className="sec-head"><div><div className="eyebrow">{t("Situation géographique")}</div><h2 style={{ marginTop: 14 }}>{t("Les 14")}<br />{t("ligues")}</h2></div><Link href="/clubs" className="sec-link">{t("Carte des clubs")}</Link></div>
      ...
      <section id="formations" className="tight"><div className="wrap"><div className="sec-head"><div><div className="eyebrow">{t("Encadrement")}</div><h2 style={{ marginTop: 14 }}>{t("Formations")}</h2></div></div>
        <div className="steps"><div className="step"><div className="step-n">{t("Entraîneurs")}</div><h3>{t("Niveaux 1 à 3")}</h3><p>{t("Cursus World Athletics dispensé par la direction technique nationale, sessions dans les ligues.")}</p></div><div className="step"><div className="step-n">{t("Officiels")}</div><h3>{t("Juges et chronométreurs")}</h3><p>{t("Homologation des juges-arbitres, starters et juges de concours ; recyclage annuel.")}</p></div><div className="step"><div className="step-n">{t("Dirigeants")}</div><h3>{t("Gestion de club")}</h3><p>{t("Affiliation, licences en ligne, organisation d'une compétition, antidopage avec l'ONADS.")}</p></div></div></div></section>
      <Partenaires  t={t} />
      <section id="contact" className="tight"><div className="wrap"><div className="sec-head"><div><div className="eyebrow">{t("Nous écrire")}</div><h2 style={{ marginTop: 14 }}>{t("Contact")}</h2></div></div>
