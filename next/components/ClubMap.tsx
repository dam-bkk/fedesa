...
export function ClubMap({ clubs, active = [], lang = "fr" }: { clubs: Club[]; active?: string[]; lang?: "fr" | "en" }) {
  ...
  return (
    <div className="cmap">
      <svg viewBox="0 0 650 470" role="img" aria-label={t("Carte des clubs affiliés au Sénégal")} onMouseLeave={() => setHover(null)}>
      ...
      <div className={`cmap-card ${cur ? "on" : ""}`} aria-live="polite">
        {cur ? (<>
          <div className="cmap-h"><b>{cur.city}</b><span className="mono">{cur.clubs.length} {cur.clubs.length > 1 ? t("clubs") : t("club")} · {t("ligue de")} {cur.region}{active.includes(cur.city) ? ` · ${t("compétition ce week-end")}` : ""}</span>{pinned && <button type="button" className="cmap-x" onClick={() => setPinned(null)} aria-label={t("Fermer")}>×</button>}</div>
          <ul>{cur.clubs.slice(0, 6).map((c) => <li key={c.id}><b>{c.name}</b><span>{c.president ? `${t("Président·e :")} ${c.president}` : ""}</span><span className="mono">{c.phone}{c.email ? ` · ${c.email}` : ""}</span><span className="dim">{c.licensees} {t("licenciés · fondé en")} {c.founded}</span></li>)}{cur.clubs.length > 6 && <li className="dim">+ {cur.clubs.length - 6} {t("autres clubs dans l'annuaire ci-dessous")}</li>}</ul>
        </>) : <p className="dim">{t("Survolez une ville pour voir ses clubs, leur adresse et leurs contacts. Cliquez pour épingler.")}</p>}
      </div>
    </div>
  );
}
