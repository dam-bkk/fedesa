  ...
  return (
    <section id="weekend" className="wk"><div className="wrap">
      ...
      <div className="wk-grid">
        <div><h3 className="poster">{t("Meilleures performances")}</h3><div className="best-list">{best.slice(0, full ? 12 : 6).map(({ c, e, r }, i) => <Link key={i} href={`/competitions/${c.id}`} className="best"><span className="best-rk">{i + 1}</span><span className="best-who"><b>{r.athlete}</b><span className="best-ev">{e.event} {e.sex === "F" ? "F" : "H"}</span><small>{r.club} · {c.name}</small></span><span className="best-mark"><span className="mono">{r.mark}</span><small>{r.points} pts{r.record ? ` · ${r.record}` : ""}</small></span></Link>)}</div></div>
