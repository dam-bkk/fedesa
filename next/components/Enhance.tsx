...
export function Enhance() {
  ...
  useEffect(() => {
    ...
    $$("[role=tablist], .tabs[role=group]").forEach((group) => { const tabs = $$<HTMLButtonElement>(".tab", group); tabs.forEach((t) => on(t, "click", () => { tabs.forEach((o) => { const a = o.closest("[role=tablist]") ? "aria-selected" : "aria-pressed"; o.setAttribute(a, o === t ? "true" : "false"); }); const r = t.getAttribute("data-r"); if (r) $$("[id^=r]", group.closest("section") ?? document).filter((p) => /^r\d+$/.test(p.id)).forEach((p) => { (p as HTMLElement).hidden = p.id !== r; }); const f = t.getAttribute("data-f"); if (f) $$("[data-cat]").forEach((i) => { (i as HTMLElement).hidden = !(f === "all" || i.getAttribute("data-cat") === f); }); })); });
