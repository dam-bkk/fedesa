        ...
        {evs.map((e, k) => (
          <div className="panel" id={`r${k + 1}`} key={k} hidden={k !== 0}>
            ...
            <div className="scrollx"><table className="tbl"><thead><tr><th style={{ width: 64 }}>{t("Pos")}</th><th>{t("Athlète")}</th><th>{t("Club")}</th><th>{t("Perf.")}</th>{e.kind === "t" && <th>{t("Vent")}</th>}<th>{t("Pts")}</th></tr></thead><tbody>
              {e.results.slice(0, 6).map((r) => <tr key={r.athleteId}><td data-th="Pos" className="c-pos"><span className={`rank ${r.rank === 1 ? "g" : r.rank === 2 ? "s" : r.rank === 3 ? "b" : ""}`}>{r.rank}</span></td><td className="c-main" data-th={t("Athlète")}><div className="who"><span className="ava">{r.athlete.split(" ").map((w) => w[0]).slice(0, 2).join("")}</span><div><div><Link href={`/athletes/${r.athleteId}`}>{r.athlete}</Link></div><div className="who-s">{r.club ? `${r.club} · ` : ""}{r.category}</div></div></div></td><td className="dim" data-th={t("Club")}>{r.club}</td><td className="c-perf" data-th={t("Perf.")}><span className="perf">{r.mark}</span></td>{e.kind === "t" && <td className="dim mono" data-th={t("Vent")}>{r.wind != null ? (r.wind > 0 ? "+" : "") + r.wind.toFixed(1) : "—"}</td>}<td className="dim mono" data-th="Pts">{r.points}</td></tr>)}
            </tbody></table></div>
          </div>
        ))}
